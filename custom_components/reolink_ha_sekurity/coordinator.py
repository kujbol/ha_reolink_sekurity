"""Coordinator for Reolink HA Sekurity."""
...
from .api import ConfigAPIView, EventDetailAPIView, EventsAPIView, MediaFileView
from .const import (
    CONF_ALARM_SENSORS,
    CONF_CAMERA_ENTITY,
    CONF_CAMERAS,
...
from .storage import MediaPathUnavailable, ensure_camera_dirs, verify_media_path
        ...
        try:
            ...
            if not self._nas_error_notified and self.error_notify_targets:
                ...
                await send_error_notification(
                    ...
                    camera_name,
                    "NAS media storage is unavailable. Recordings are NOT being saved. "
        ...
        try:
            ...
            if not self._nas_error_notified and self.error_notify_targets:
                ...
                await send_error_notification(
                    ...
                    self.error_notify_targets,
                    camera_name,
                    "NAS media storage became unavailable during recording. "
                    "Check Settings → System → Storage.",
                )
        except Exception:
            _LOGGER.exception(
